...
List = function(params) {
	...
	function onSelectItemClick(ev, tmpl) {
		debug('inplace:onSelectItemClick');
	...
	*/
	...
	function modify(ev, tmpl) {
		...
		// Get the form and add a loading class if it's an async item
		var $form = $('form', item.view.firstNode());
		if (item.async) {
		...
		Meteor.call(item.method, data, this, item.name, function(err, res) {
			if (err) {
				// TODO pop up error, major thrown error... not sure what here
				// for now same as cancel
				closeEditor(ev, tmpl);
				return;
			}

			// If result is good close the editor
			if (res === true) {
				closeEditor(ev, tmpl);
				return;
			}
			...
			// Determine which keys are server only
			var serverKeys = _(res).filter(function(key){
				return !_(validationContext.invalidKeys()).findWhere(key);
			});

			// Add server only keys to validation context
			validationContext.addInvalidKeys(serverKeys);

			// Update the form classes for displaying error messages
			tmpl.$('form').addClass('error');
			_(validationContext.invalidKeys()).each(function(key){
				tmpl.$('*[name="'+key.name+'"]').parent('.field').addClass('error');
			});

			// Remove the form loading class
